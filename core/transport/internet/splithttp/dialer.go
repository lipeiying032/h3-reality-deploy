...
func decideHTTPVersion(tlsConfig *tls.Config, realityConfig *reality.Config) string {
	if tlsConfig != nil {
		if len(tlsConfig.NextProtocol) == 1 {
			switch tlsConfig.NextProtocol[0] {
			case "h3":
				return "3"
			case "http/1.1":
				return "1.1"
	...
	if realityConfig != nil {
		if len(realityConfig.Alpn) == 1 && realityConfig.Alpn[0] == "h3" {
			return "3"
		}
		return "2"
	}
...
func createHTTPClient(dest net.Destination, streamSettings *internet.MemoryStreamConfig) DialerClient {
	...
	if httpVersion == "3" {
		...
		if realityConfig != nil {
			// C-gamma REALITY over QUIC: the authentication payload rides in
			// the ClientHello random field (stage-2), so the ClientHello
			// stays standard otherwise: session_id=0, no custom extensions,
			// 5 supported groups. The client accepts the server's Dest
			// certificate chain. The handshake runs on the vendored
			// xtls/reality TLS fork in data-plane-auth mode: the fork seals
			// the REALITY payload into the random field and skips the TLS
			// 1.3 CertificateVerify signature check, which stock crypto/tls
			// cannot do (the server presents the Dest chain but signs
			// CertificateVerify with a throwaway key).
			if gotlsConfig == nil {
				gotlsConfig = &gotls.Config{}
			}
			gotlsConfig.InsecureSkipVerify = true
			// Go 1.26's stock crypto/tls enables three PQ hybrid groups by
			// default (7 groups in the ClientHello). Pin the QUIC client to
			// the Go 1.24/1.25 baseline of 5 groups (X25519MLKEM768 + four
			// classic curves) so the handshake fingerprint matches a
			// widely-deployed regular quic-go client.
			gotlsConfig.CurvePreferences = []gotls.CurveID{
				gotls.X25519MLKEM768, gotls.X25519, gotls.CurveP256, gotls.CurveP384, gotls.CurveP521,
			}
			// masterKeyLog also applies to the QUIC (C-gamma) client path, so
			// captures of the HTTP/3 data plane can be decrypted for
			// inspection.
			if w := reality.KeyLogWriterFromConfig(realityConfig); w != nil {
				gotlsConfig.KeyLogWriter = w
			}
			quicFactoryTemplate := &goreality.Config{
				DataPlaneAuth: true,
				PublicKey:     realityConfig.PublicKey,
				ShortId:       realityConfig.ShortId,
			}
			// An explicitly configured fingerprint switches the QUIC
			// ClientHello to the uTLS fingerprint path (the fork builds the
			// ClientHello from the uTLS Chrome spec and still seals the
			// REALITY payload into the random field). Empty or unknown
			// fingerprints keep the stock Go ClientHello, so the default
