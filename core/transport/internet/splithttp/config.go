...
func (c *Config) GetRequestHeader() http.Header {
	...
	for k, v := range c.Headers {
		// An empty value in the headers config suppresses the
		// browser-masquerade default for that header (e.g. "dnt": ""
		// removes the DNT: 1 that the Chrome profile would otherwise add).
		// The key is deleted after the defaults are applied.
		if v == "" {
			suppressed[k] = true
		}
		header.Add(k, v)
	}
	// Navigation-style profile: the outer XHTTP request is a browser-like
	// top-level form POST (full Accept list, upgrade-insecure-requests,
	// sec-fetch-mode: navigate, no pragma/dnt/*/* fetch defaults).
	utils.TryDefaultHeadersWith(header, "nav")
