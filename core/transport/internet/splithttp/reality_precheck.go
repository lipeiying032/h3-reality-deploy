...
// clientHelloServerName extracts the SNI (server_name extension, RFC 6066)
// from a raw TLS ClientHello handshake message (including its 4-byte
// handshake header). It returns "" when the message is malformed or carries
// no server_name extension. The parse is intentionally minimal: only the
// fixed header, session_id/cipher_suites/compression_methods lengths and the
// extension list are walked, exactly like the reality fork's unmarshal.
func clientHelloServerName(hello []byte) string {
	body := hello
	if len(body) >= 4 && body[0] == 0x01 { // typeClientHello + 3-byte length
	...
	extsLen := int(binary.BigEndian.Uint16(body))
	body = body[2:]
	if len(body) < extsLen {
		return ""
	}
	...
	for len(body) >= 4 {
		...
		if extType == 0x0000 { // extensionServerName
			return serverNameFromList(body[:extLen])
		}
		body = body[extLen:]
	}
	return ""
...
// serverNameFromList parses a server_name extension payload (RFC 6066
// Section 3): a 2-byte name_list length followed by name_type(1) +
// name_len(2) + name entries. Only name_type 0 (host_name) is returned.
func serverNameFromList(data []byte) string {
	if len(data) < 2 {
		return ""
	}
