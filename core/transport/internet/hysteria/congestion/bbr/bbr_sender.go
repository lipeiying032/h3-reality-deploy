...
// OnPacketsLost implements the CongestionControlEx interface.
// Cleans up obsolete packet state data in the bandwidth sampler.
func (b *bbrSender) OnPacketsLost(leastUnacked congestion.PacketNumber) {
	b.sampler.RemoveObsoletePackets(leastUnacked)
}

// OnAppLimited implements the CongestionControlEx interface.
// Notifies the sampler when the application is app-limited.
