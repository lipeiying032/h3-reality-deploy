...
type REALITYConfig struct {
	...
	// H3CertificateFile / H3KeyFile are optional PEM files for the C-gamma
	// XHTTP/3 QUIC listener: stock crypto/tls clients require the server to
	// hold the private key of the certificate it presents, so the operator
	// points these at a certificate+key they own (typically the same ones the
